...
if (isset($_POST['email']) && isset($_POST['password'])) {
    if( ($_POST['email'] != '') && ($_POST['password'] != '') ) {
        ...
        $sql_init = pg_prepare($dbconn, "login_", "select i.u_courriel,i.u_pwd, i.u_id_session , i.u_nom||' '||i.u_prenom as nom_prenom, i.u_id from $users i where (i.u_pwd = md5( $1 ) and i.u_courriel = $2 )");
        
        //execute la requete dans le moteur de base de donnees  
        $qout = pg_execute($dbconn,"login_",array($_POST['password'],$_POST['email'])) or die (pg_last_error());
