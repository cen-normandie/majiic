...
if (pg_num_rows($res) > 0) {
    ...
    $sql_update = "UPDATE $parcelles
        ...
        id_doc_gestion = $3,
        id_ore = $4,
        id_unique = $5 || COALESCE(NULLIF($6, 'ø'), NULLIF($7, 'ø'), NULLIF($8, 'ø'), '')
        WHERE SPLIT_PART(id_unique,'|',2) = $9";

    $prepared_update = pg_prepare($dbconn, "update_parcelle", $sql_update);

    $id_unique_base = $p_site . '|' . $p_insee . $p_prefixe . str_pad($p_section, 2, '0', STR_PAD_LEFT) . str_pad($p_num, 4, '0', STR_PAD_LEFT) . '|';
    ...
    $sql_insert = "INSERT INTO $parcelles (
        ...
        VALUES (
        $1 || COALESCE(NULLIF($2, 'ø'), NULLIF($3, 'ø'), NULLIF($4, 'ø'), ''),
        $5,
        $6,
        $7,
        $8,
        ...
        $10,
        $11,
        COALESCE(NULLIF($2, 'ø'), NULLIF($3, 'ø'), NULLIF($4, 'ø'), '')
        )";
